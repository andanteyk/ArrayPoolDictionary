...
namespace ParallelDungeon.Rogue.Serialization
{
    ...
    public class HopscotchDictionary<TKey, TValue> :
        ...
        where TKey : notnull
    {
        ...
        private (int hashCode, int bucketIndex, int entryIndex) GetEntryIndex(TKey key)
        {
            ...
            for (int i = 0; i < 64; i++)
            {
                // TODO: more faster when use lzcnt (or similar)?
                if ((bitmap & (1ul << i)) != 0)
                {
        ...
        public HopscotchDictionary() : this(16) { }
        public HopscotchDictionary(int capacity)
        {
            m_Entries = ArrayPool<Entry>.Shared.Rent(capacity);
            m_Entries.AsSpan().Clear();
            m_Count = 0;
        }
        public HopscotchDictionary(IDictionary<TKey, TValue> source) : this(source.Count)
        {
            if (source is HopscotchDictionary<TKey, TValue> cloneSource)
            {
        ...
        public HopscotchDictionary(IEnumerable<KeyValuePair<TKey, TValue>> source) : this(source.Count())
        {
            foreach (var element in source)
            {
                Add(element);
        ...
        {
            private readonly HopscotchDictionary<TKey, TValue> m_Parent;
            private int m_Index;

            internal Enumerator(HopscotchDictionary<TKey, TValue> parent)
            {
                m_Parent = parent;
                m_Index = -1;
            }
